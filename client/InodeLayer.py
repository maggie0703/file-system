...
interface0 = BlockLayer.BlockLayer0()
interface1 = BlockLayer.BlockLayer1()
interface2 = BlockLayer.BlockLayer2()
interface3 = BlockLayer.BlockLayer3()
...
class InodeLayer0():
    ...
    def INODE_TO_BLOCK(self, inode, offset):
        ...
        else: return interface0.BLOCK_NUMBER_TO_DATA_BLOCK(block_number)
    ...
    def free_data_block(self, inode, index):
        for i in range(index, len(inode.blk_numbers)):
            interface0.free_data_block(inode.blk_numbers[i])
    ...
    def write(self, inode, offset, data):
        ...
        for i in range(0, total_blocks):     #WRITING BLOCKS
            new_valid_block_number = interface0.get_valid_data_block()
            inode.blk_numbers[i] = new_valid_block_number
            interface0.update_data_block(new_valid_block_number, blocks[i])
...
class InodeLayer1():
    ...
    def INODE_TO_BLOCK(self, inode, offset):
        ...
        block_number = self.INDEX_TO_BLOCK_NUMBER(inode, index)
        if block_number == -1: return ''
        else: return interface1.BLOCK_NUMBER_TO_DATA_BLOCK(block_number)
    ...
    def free_data_block(self, inode, index):
        for i in range(index, len(inode.blk_numbers)):
            interface1.free_data_block(inode.blk_numbers[i])
            inode.blk_numbers[i] = -1
    ...
    def write(self, inode, offset, data):
        ...
        for i in range(0, total_blocks):     #WRITING BLOCKS
            new_valid_block_number = interface1.get_valid_data_block()
            inode.blk_numbers[i] = new_valid_block_number
            interface1.update_data_block(new_valid_block_number, blocks[i])
...
class InodeLayer2():
    ...
    def INODE_TO_BLOCK(self, inode, offset):
        ...
        block_number = self.INDEX_TO_BLOCK_NUMBER(inode, index)
        if block_number == -1: return ''
        else: return interface2.BLOCK_NUMBER_TO_DATA_BLOCK(block_number)
    ...
    def free_data_block(self, inode, index):
        for i in range(index, len(inode.blk_numbers)):
            interface2.free_data_block(inode.blk_numbers[i])
            inode.blk_numbers[i] = -1
    ...
    def write(self, inode, offset, data):
        ...
        for i in range(0, total_blocks):     #WRITING BLOCKS
            new_valid_block_number = interface2.get_valid_data_block()
            inode.blk_numbers[i] = new_valid_block_number
            interface2.update_data_block(new_valid_block_number, blocks[i])
...
class InodeLayer3():
    ...
    def INODE_TO_BLOCK(self, inode, offset):
        ...
        block_number = self.INDEX_TO_BLOCK_NUMBER(inode, index)
        if block_number == -1: return ''
        else: return interface3.BLOCK_NUMBER_TO_DATA_BLOCK(block_number)
    ...
    def free_data_block(self, inode, index):
        for i in range(index, len(inode.blk_numbers)):
            interface3.free_data_block(inode.blk_numbers[i])
            inode.blk_numbers[i] = -1
    ...
    def write(self, inode, offset, data):
        ...
        for i in range(0, total_blocks):     #WRITING BLOCKS
            new_valid_block_number = interface3.get_valid_data_block()
            inode.blk_numbers[i] = new_valid_block_number
            interface3.update_data_block(new_valid_block_number, blocks[i])
